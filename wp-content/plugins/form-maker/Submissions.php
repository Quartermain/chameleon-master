...
function show_submits() {
  ...
  $query = "SELECT * FROM " . $wpdb->prefix . "formmaker_submits " . $where;
	$rows_paypal = $wpdb->get_results($query);
	// if (!$rows_paypal) {
    // return false;
  // }
  // Display function.
  html_show_submits($rows, $forms, $lists, $pageNav, $sorted_labels, $label_titles, $rows_ord, $filter_order_Dir, $form_id, $sorted_labels_id, $sorted_labels_type, $total_entries, $total_views, $where, $where_choices, $rows_paypal, $sort);
}
...
function editSubmit($id) {
  ...
  // Display function.
  html_editSubmit($rows, $label_id, $label_order_original, $label_type, $ispaypal);
}
