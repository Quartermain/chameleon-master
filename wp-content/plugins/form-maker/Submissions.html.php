...
function html_show_submits($rows, $forms, $lists, $pageNav, $labels, $label_titles, $rows_ord, $filter_order_Dir, $form_id, $labels_id, $sorted_labels_type, $total_entries, $total_views, $where, $where_choices, $rows_paypal, $sort) {
...
jQuery(document).ready(function ($) {
  ...
  thickDims = function () {
    ...
    w = (tbWidth && tbWidth < W - 90) ? tbWidth : W - 700;
    h = (tbHeight && tbHeight < H - 60) ? tbHeight : H - 100;
...
<tr style="line-height:inherit !important;">
  ...
  <?php print_html_nav($pageNav['total'], $pageNav['limit'], ((isset($serch_filds)) ? $serch_filds : ''));
  ?>
...
<tr>
  ...
  for ($i = 0; $i < count($labels); $i++) {
    ...
    if ($sorted_labels_type[$i]=='type_paypal_payment_status') {
			$ispaypal = TRUE;
      ?>
			<th style="<?php  echo $styleStr; ?>" id="<?php  echo $labels_id[$i] . '_fc';?>" class="<?php  echo $labels_id[$i] . '_fc';
        if ($sort["sortid_by"] == $labels_id[$i] . "_field") echo $sort["custom_style"] . '"';
        else echo $sort["default_style"] . '"'; ?> ">
        <a href="javascript:ordering('<?php echo $labels_id[$i] . "_field"; ?>',<?php if ($sort["sortid_by"] == $labels_id[$i] . "_field") echo $sort["1_or_2"]; else echo "1"; ?>)"><span><?php echo $field_title ?></span><span class="sorting-indicator"></span></a>
      </th>
      <?php
      if (strpos($lists['hide_label_list'], '@payment_info@') === FALSE) {  
        $styleStr2 = '';
        ...
        $styleStr2 = 'display:none;';
      }
			echo '<th class="payment_info_fc" '.$styleStr2.'>Payment Info</th>';
		}
		else {
			?>
			<th style="<?php  echo $styleStr; ?> <?php echo (($sorted_labels_type[$i] == 'type_star_rating') ? 'width:140px' : ''); ?>" id="<?php  echo $labels_id[$i] . '_fc';?>" class="<?php  echo $labels_id[$i] . '_fc';
        if ($sort["sortid_by"] == $labels_id[$i] . "_field") echo $sort["custom_style"] . '"';
        else echo $sort["default_style"] . '"'; ?> ">
        <a href="javascript:ordering('<?php echo $labels_id[$i] . "_field"; ?>',<?php if ($sort["sortid_by"] == $labels_id[$i] . "_field") echo $sort["1_or_2"]; else echo "1"; ?>)"><span><?php echo $field_title ?></span><span class="sorting-indicator"></span></a>
      </th>
  ...
  for ($i = 0; $i < count($labels); $i++) {
    ...
    switch($sorted_labels_type[$i]) {
      ...
      case 'type_paypal_payment_status':
        ...
        break;
      default:
        echo '<th class="'.$labels_id[$i].'_fc" '.$styleStr.'>'.'<input name="'.$form_id.'_'.$labels_id[$i].'_search" id="'.$form_id.'_'.$labels_id[$i].'_search" type="text" value="'.$lists[$form_id.'_'.$labels_id[$i].'_search'].'"  onChange="this.form.submit();" >'.'</th>'; break;			
    }
  }
  ?>
  <th></th>
    ...
    if ($ispaypal) {
        ...
        $styleStr = 'style="display:none;"';
			...
			echo '<td align="center" class="payment_info_fc" '.$styleStr.'>
              <a class="thickbox-preview" href="' . admin_url('admin-ajax.php?action=paypal_info&id=' . $i . '&TB_iframe=1') . '">
                <img src="' . plugins_url("images/info.png",__FILE__) . '" />
              </a>
            </td>';
		}
    ?>
...
function html_editSubmit($rows, $labels_id, $labels_name, $labels_type, $ispaypal) {
  ?>
